---
title: YouCompleteMe-readme_zh
categories:
  - Vim
date: 2018-03-14 22:23:27
updated: 2018-03-14 22:23:27
tags: 
  - Vim
  - YouCompleteMe
  - YCM
...
首先很认真的阅读针对你系统的[安装说明一节](#installation)。
我们建议你使用代码中提供的 `install.py`————完整的安装指导不常用，进阶的使用场景和绝大数用户都应该使用 `install.py`。

如果服务器没有启动起来，同时你看到一个 "YouCompleteMe unavailable" 错误，检查 [Troubleshooting][wiki-troubleshooting] 指南。

然后检查你正在使用的语义补全的 [User Guide](#user-guide) 一节。 对于 C/C++/Objective-C/Objective-C++/CUDA ，你 **必须** 阅读 [this
section](#c-family-semantic-completion) 一节。

最好，检查[FAQ][wiki-faq].

如果在阅读完安装和用户指南并检查了 FAQ 后，你依然还有问题，查看 [contacts](#contact) 节来联系我们。
get in touch.

你  **不要** 在 freenode 中的 #vim 节点去寻求支持。直接用 [contact details](#contact) 中的方式联系维护者。
...
- 一个基于标识符的引擎，可以和任何语言工作。
- 一个很给力的基于 [clangd][] 的引擎， 对 C/C++/Objective-C/Objective-C++/CUDA (当前叫做 C 家族) 提供原生的语义全补
- 基于 [Jedi][]，针对 Python2，3 的补全
- 基于 [OmniSharp-Roslyn][] 的 C# 补全引擎。
- 基于 [Gopls][] 的 GO 补全
- 基于 [TSServer][] 的 JS/TS 补全
- 基于 [rust-analyzer][] Rust 补全
- 基于 [jdt.ls][] JAVA 补全
- 一个 [generic Language Server Protocol implementation for any language](#plugging-an-arbitrary-lsp-server)
- 一个基于 omnifuc 的完成器，它使用 Vim 的 omnicomplete 系统中的数据来提供其他语言的语义补全。
...
一个需要注意的事情就是这个补全 **过滤并不是基于输入的是一个字符串前缀**（虽然这也能工作）。输入需要是一个补全的 _[subsequence][]_。这就是说，任何输入的字符都需要在补全字符串中以相同的顺序出现。因此 `abc` 是 `xaybgc` 的一个子序列，但不是 `xbyxaxxc` 的。在过滤后，一个负责的排序系统会将补全字符串进行排序。
...
YCM 也在很多语言中提供  [semantic IDE-like features](#quick-feature-summary) 包括:

- 当在函数调用中键入参数时显示签名帮助（参数提示） (Vim only)
...
**Remember:** YCM 是一个编译好组件的插件。如果你使用  Vundel **更新**了 YCM
和 `ycm_core` 库的 API 变更了（很少见）, YCM 将会提示你进行重新编译。那你就需要重新进行安装流程了。
...
启用所有东西的简单方式就是加上 `--all` 标志。我们需要手动添加 `--clangd-completer`。为了安装所有的语言特性，确保`xbuild`, `go`, `node` and `npm` 都安装 了，且都在 `PATH` 变量中。
...
> YCM 原来使用的基于 `libclang` ，不过用户都应该切换到 `cland`，因为它提供了更多的特性和更好的性能。当依赖`.ycm_extra_conf.py` 中的 `override_filene`  用户，就需要停留在 `libclang` 引擎。如何停留在老引擎，看 [the wiki][libclang-instructions].
...
如果没找到  [`.ycm_extra_conf.py`](#option-2-provide-the-flags-manually) ，YouCompleteMe自动尝试加载一个存在的编译数据库。
...
[`g:ycm_global_ycm_extra_conf`](#the-gycm_global_ycm_extra_conf-option) 提供一个全局参数，这将作为一个回退。.为了阻止执行到恶意的代码，YCM 会每加载一个 `.ycm_extra_conf.py` 就进行询问。这可以禁用，也可以设置黑白名单。 See
